    ...
    implements Runnable
{
    ...
    public void run()
    {
		...
		if ((canvas.mk!=null)&&(canvas.mk.connected)&&(canvas.settings.webreporter)&&(last_send_id--<0))
			...
			String request_str="http://ligi-tec.appspot.com/update_ufo?ubatt="+canvas.mk.UBatt()+"&ufo_id="+canvas.settings.dubwise_id + "&sender=" + canvas.mk.SenderOkay() + "&alt=" + canvas.mk.Alt();
